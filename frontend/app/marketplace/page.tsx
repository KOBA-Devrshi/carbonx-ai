...
function Overview({ credit, reload }: { credit: CreditDetail; reload: () => void }) {
  ...
  const [marketPrice] = useState<number>(() => {
    return Math.round((credit.available_tco2e > 0 ? 35.00 : 25.00) * 100) / 100;
  });

  async function buy(e: React.FormEvent) {
    e.preventDefault();
    if (!proofFile) {
      setMsg("Error: Immutable proof document (funds verification / registry statement) is required to buy.");
      return;
    }
    ...
    try {
      setTimeout(async () => {
        ...
        setMsg(`Transaction confirmed — $${res.total} paid at live market parity for ${qty} tCO2e. Proof verified.`);
        reload();
        setBusy(false);
      }, 1000);
    } catch (err: any) { 
  ...
  const fairValueEstimate = Math.round((marketPrice * 0.85) * 100) / 100;
  const priceDiff = marketPrice - fairValueEstimate;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        ...
        {/* Live Market Price vs Platform Purchase Block with Required Proof */}
        <div className="card border border-border bg-surface">
          <div className="font-sans font-semibold mb-3">Live Market vs. Platform Execution</div>
          
          <div className="bg-surface2 p-3.5 rounded-lg mb-4 border border-border">
            <div className="flex justify-between text-[13px] mb-1.5">
              <span className="text-textDim">External Live Market Price:</span>
              <span className="font-mono font-semibold">${marketPrice.toFixed(2)} / tCO2e</span>
            </div>
            <div className="flex justify-between text-[13px] mb-2">
              <span className="text-textDim font-medium">CarbonX Risk-Adjusted Value:</span>
              <span className="font-mono font-bold text-accent">${fairValueEstimate.toFixed(2)} / tCO2e</span>
            </div>
            <div className="text-[11.5px] text-textFaint pt-2 border-t border-border">
              {priceDiff > 0 ? `📊 Live market is trading $${priceDiff.toFixed(2)} higher than risk-adjusted fair value.` : `📉 Favorable platform pricing relative to live market.`}
            </div>
          </div>

          <form onSubmit={buy} className="space-y-3">
            <div className="field mb-2">
              <label>Purchase Quantity (tCO2e)</label>
              <input 
                type="number" 
                value={qty} 
                onChange={(e) => setQty(parseFloat(e.target.value) || 0)} 
                className="w-full bg-surface2 border border-border p-2 rounded-lg text-[13px]"
              />
            </div>

            <div className="field mb-3">
              <label className="text-[12.5px] font-medium text-textDim">Upload Required Proof (Audit / Wire Confirmation)</label>
              <input 
                type="file" 
                accept=".pdf,image/*" 
                onChange={(e) => setProofFile(e.target.files?.[0] || null)}
                className="w-full border border-border bg-surface2 p-2 rounded-lg text-[12px]"
                required
              />
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
              <button 
                type="submit" 
                disabled={busy || credit.integrity_state === "INTEGRITY_REVIEW" || credit.available_tco2e <= 0} 
                className="btn btn-primary btn-sm w-full disabled:opacity-50"
              >
                {busy ? "Processing Secure Trade..." : `Buy at Live Benchmark ($${(marketPrice * qty).toFixed(2)})`}
              </button>
            </div>

            {credit.integrity_state === "INTEGRITY_REVIEW" && (
              <div className="text-danger text-[12px] mt-2">Trading restricted — credit is locked under Integrity Review.</div>
            )}
            {msg && <div className={`text-[12.5px] mt-2 font-mono ${msg.includes("Error") ? "text-danger" : "text-accent"}`}>{msg}</div>}
          </form>
        </div>
      </div>
    </div>
    ...
    <div className="card">
      <div className="flex justify-between items-center flex-wrap gap-3">
        <div>
          ...
          <div className="text-textDim text-[13px]">The Defender argues the claim is supported; the Auditor challenges it point by point. A REJECT verdict automatically triggers the Integrity Lock.</div>
        </div>
      ...
      {autoLocked && (
        <div className="mt-4 px-4 py-3 rounded-lg bg-danger/10 border border-dangerDim flex items-center gap-2">
          ...
          <div className="text-[13px] text-danger">
            <span className="font-semibold">Integrity Lock auto-triggered.</span> The AI Auditor's REJECT verdict has restricted trading on this credit.
          </div>
        </div>
      )}
